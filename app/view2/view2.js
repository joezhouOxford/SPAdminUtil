 ...
 */
angular.module('myApp.view2', ['ngRoute', 'ngMaterial','ngSanitize','angular-bind-html-compile'])
    ...
    .config(['$routeProvider', '$mdIconProvider', '$mdDateLocaleProvider', function ($routeProvider, $mdIconProvider, $mdDateLocaleProvider) {
        ...
        $mdIconProvider
            .icon('openLink', siteRootRelativePath + 'img/icons/ic_open_in_new_black_24px.svg', 24);
    ...
    .controller('View2Ctrl', ['$scope', '$q', '$log', '$timeout', '$mdDialog', '$window', function ($scope, $q, $log, $timeout, $mdDialog, $window) {
        var goLive = true;
        var listColumnTitleArray = ["L1 Name","L4 Legal Name","Client CIS", "Operational Point Zero Date", "All Accounts On Buses", "New Bank Accounts Setup", "L4Client Indicated Cash Date", "L4Client Indicated Trade Date", "Client Mandated New Bank", "Detailed Onboarding Plan In Place"];
        ...
        $timeout(function () {
            angular.forEach(listColumnTitleArray, function (columnTitle) {
                var def;
                if(columnTitle=="Client CIS")
                {
                    def={
                    name:columnTitle,
                        headerTooltip: columnTitle,
                    cellTemplate:siteRootRelativePath+'view2/customHtml.html',
                        width:100
                     };
                }
                else {
...
//single row selection
        ...
        function appendItemIntoGrid(listItem, listTitle) {
            ...
            angular.forEach(listColumnInternalNameArray, function (colInternalName) {
                ...
                var cellValue = listItem.get_item(colInternalName);
                if (colInternalName.indexOf("Date") > -1 || colInternalName.indexOf("ClientMandatedNewBank") > -1) {
                    var cellDate = moment(cellValue);
                    if (cellDate.isValid()) {
                        cellValue = cellDate.format("YYYY-MM-DD");
                    }
                }
                else if(colInternalName=="Title")
                {
                    var listName=listTitle;
                    if(listName=="L4Global")
                    {
                        listName="L4Inbound";
                    }
                    //postSurvey('L4 Edit Item', '"+cellValue+"');
                    cellValue="<md-icon md-svg-icon=\"openLink\" aria-label=\"open L4\" onclick=\"javascript:postSurvey('L4 Edit Item', '"+cellValue+"'); NewItem2(event,'/teams/EOCBM/Lists/"+listName+"/EditForm.aspx?ID="+id+"&IsDlg=1');\"></md-icon>" + cellValue;
                }

                gridItem[colTitle] = cellValue;
        ...
        function loadDummyData() {
            ...
            for (var i = 1; i < 21; i++) {
                var fakeListItem = {"id": i, "ListTitle": "L4Dev"};
                fakeListItem.get_item = function (fieldName) {
                    switch (fieldName) {
                        case "ID":
                            return i;
                            break;
                        case "AllAccountsOnBuses":
                            return Date();
                            break;
                        default:
                            return i + " " + fieldName + " Test data for";
                            break;
                    }

                }
                appendItemIntoGrid(fakeListItem, "L4Dev")
            }
        ...
        var resizeView = function () {
            var offsetheight = 20;
            var topNavHeight = document.getElementsByClassName('topNav')[0].offsetHeight;
            var viewHeight = $window.innerHeight - topNavHeight - offsetheight;

            $log.log("viewHeight:" + viewHeight);
            angular.element(document.getElementsByClassName('angular-View')[0]).css('height', viewHeight + 'px');
        }
        var resizeGrid = function () {
           ...
           /* $log.log("windows width:" + $window.innerWidth);
            ...
            var offSet = 5;
            var resultGridPaneWidth= document.querySelectorAll('.resultGridPane')[0].clientWidth;
            var gridWidth = resultGridPaneWidth - offSet;
            $log.log("gridWidth:" + gridWidth);
            angular.element(document.getElementsByClassName('myGrid')[0]).css('width', gridWidth + 'px');
            var heightOffSet = 3;
            var gridHeight = document.querySelectorAll(".resultGridPane")[0].offsetHeight - heightOffSet;
